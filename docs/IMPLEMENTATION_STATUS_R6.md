...
# R6 补丁 8：搜索结果标出来源，并支持按来源筛选

## 问题

搜「魔女宅急便」返回 Bangumi 10 条 + TMDB 2 条，混在一起。同名条目一大串
（1989 动画版、2014 真人版、各种关联条目），光看标题分不清哪条来自哪个库，
也就没法判断该选哪个。顶部那排 `Bangumi 10 条 / TMDB 2 条` 当时只是状态显示，不可点。

## 这算不算违背「用户不需要选数据源」

不算。原则针对的是**搜索入口**：仍然只有一个搜索框，两个源仍然都会被搜，
用户不需要在搜索**之前**决定用哪个库。

来源现在是结果上的**一条可见信息 + 一个可选筛选**——发生在拿到结果**之后**，
用来消歧义。这两件事是不同的。

## 做法

- 每条外部结果加一个来源徽章（Bangumi 洋红 / TMDB 墨绿），扫一眼就能分辨，
  不用逐条读文字。
- 顶部那排 chip 升级成可点击的筛选：点 `Bangumi 10 条` 只看 Bangumi，
  再点或点「← 全部」回到全部。**未配置 / 失败的源保持不可点**——它们没有结果可筛，
  仍然只做状态提示。
- 换查询词时筛选自动清空。

### 一个容易踩的坑

`itemMarkup` 传给回调的是**数组下标**。外部组在筛选状态下渲染的是过滤后的数组，
所以 `selectWorkSearchCandidate` 必须用**同一份过滤后的数组**取候选——
否则筛选状态下点第 1 条，选中的是未过滤数组里的第 1 条（完全是另一部片）。
`filterCandidatesBySource()` 是纯函数，渲染和回调共用它，从结构上避免了两边不一致。

---

# R6 补丁 9：海报按发行地区自动择优（方案 A）

## 问题

`TMDB_LANGUAGE=zh-CN` 会让 TMDB 优先返回中文海报。像 Birdman 这种没在中国大陆
上映的欧美片，拿到的可能是港台中文版海报，和作品原本的视觉设计相去甚远。

## 成本为什么低

TMDB 的 `images` 可以走 `append_to_response` 一起带回来，**不产生额外请求**。
唯一要注意的是必须显式给 `include_image_language`，否则只会返回当前 `language`
那一档。

## 回落链（不是硬编码的绝对规则）

```
1. 出品国对应语言        JP→ja  US/GB→en  FR→fr  KR→ko …
2. 原始语言              original_language（出品国表里没有时的近似）
3. 无文字海报            iso_639_1 为 null —— 通常最接近原始视觉设计
4. 英语
5. TMDB 给的 poster_path 当前 language 那一档
6. 候选里的第一张
```

任何一档拿不到都只是往下走，**不会失败**。`COUNTRY_POSTER_LANGUAGE` 只覆盖常见
出品国，缺的随时可加；查不到的国家自然走后面的回落。

同一档内有多张时按 `vote_average` 取高的——用 TMDB 自己的社区评分，比随机取稳。

非法 `file_path` 在挑选阶段就被过滤掉（复用 `isValidTmdbPosterPath`），
不会被拼进图片代理的上游 URL。

## 没有做的部分（方案 B）

「用户手动更换封面」这一步**没有实现**。它需要：存候选海报或每次打开面板重新拉一次
详情、作品页加海报网格面板、Work 上加 `poster_locked` 让自动规则不再覆盖用户的选择。
成本大约是方案 A 的两倍多，且「用户选择优先」这条状态要贯穿后续所有元数据刷新。

建议先用方案 A 跑一段时间，自动规则挑得满意就不必做 B。
...
用户反馈：AI 生成的卡片只是把感想原文**机械切分转抄**成几张，没有做分析与提炼，
不构成"整理成更好的记录"。

这与产品定义里「AI 并非先替用户写影评，而是先帮助用户整理记忆素材」的目标有差距——
当前实现更接近"分段"而不是"整理"。

**本轮未处理**，相关代码在 `functions/api/ai/analyze.js` 与 `src/ai-providers.js`
（提示词与卡片结构），以及 `src/domain.js` 的 `deterministicAnalysis()`（无 AI 时的
本地兜底，目前就是按句切分）。建议由专门负责卡片设计的窗口重做提示词与卡片类型体系。

## 捕获流程的外部匹配仍是 Bangumi 单源

全局「＋ 开始记录」用的是 R2 时代的 `runCaptureBangumiMatch()`，只搜 Bangumi，
与片单那套统一搜索（本地 + Bangumi + TMDB 并行）是两条独立代码路径。

从作品页进入的记录流程已经完全不需要它（补丁 7 的 `lockedWork`），
所以这条路径只影响"记录一部库里还没有的新片"。合并两条路径需要把捕获流程的
匹配 UI 换成统一搜索面板，改动面不小，本轮未做。

---

# R6 补丁 10：捕获流程与详情页匹配也改成多源

## 问题

上一轮我把这条只记成了「待办」而没有修——这是我的疏忽，它本来就该修。

全局「＋ 开始记录」和详情页的「待确认作品」面板，用的都是 R2 时代只搜 Bangumi 的
代码路径。当初那样写是因为最早只接了一个源、且以看动画电影为主，先拿一个源试效果。
但只要看真人电影，Bangumi 常常根本没有条目，用户就卡在"匹配不到"——
**而这正是引入 TMDB 本来要解决的问题**。

## 涉及的三条路径，现在走同一条统一搜索

| 入口 | 之前 | 现在 |
|---|---|---|
| 片单「添加作品」 | 本地 + Bangumi + TMDB | 不变 |
| 捕获流程（票务粘贴 / 场景选择填片名） | **只有 Bangumi** | 本地 + Bangumi + TMDB |
| 详情页「待确认作品」（新记录后自动触发） | **只有 Bangumi** | Bangumi + TMDB |
| 作品页「记录这次观看」 | — | 不搜（补丁 7 的 `lockedWork`，Work 已确定） |

## 数据模型上的两处关键改动

### `applyCandidateToWork()`（`src/domain.js`，新增）

`promoteWorkToMatched()` 的签名是 `(work, subjectId, bangumiData)`——**写死了 Bangumi**。
新函数接受统一候选（local / bangumi / tmdb 同形），只做两件事：把候选携带的**全部**
external id upsert 进去、用候选元数据补全 Work 上还空着的字段。**不改 work.id**。

几条刻意的保守规则：

- 候选判断不出类型时**保留 Work 原有的类型**，不倒退成 `unspecified`
- **已有海报不被覆盖**——用户可能已经挑过，或先前的源画质更好
- `primary_source` 一旦确定就不被后来的源改写
- 上映日一律按 `region: "unknown"` 落库（Bangumi 的 `date` 和 TMDB 的 `release_date`
  都不带地区语义），由用户在作品页认领

### `captureContext.selectedCandidate`

以前只存一个 `subjectId`，装不下 tmdb_id、海报路径、时长这些信息——TMDB 候选一旦被
选中，落库时这些全丢。现在存**整条统一候选**，`finishCompose` 直接复用
`resolveOrCreateWorkFromCandidate()`，也就是片单添加走的**同一条**落库路径：
补 TMDB 详情 → 按发行地区挑海报 → 相同 external id 不重复建 Work。

`ctx.subjectId` 仍然同步维护一份，因为草稿卡海报等老代码还在读它。

### 冲突检测按候选自己的源来查

`confirmWorkMatch()` 里的重复作品检测原本写死 `externalRefId(item, "bangumi")`，
现在按 `candidate.source` 查——TMDB 候选撞车时同样能正确合并。

## 被静态检查拦下的一个错误

改完后 `applyCandidateToWork` 忘了在 `src/app.js` 里 import，全量未定义调用扫描
当场报出来了。这正是补丁 2 之后加强那个检查的价值——同一类错误（改了模块接口但
调用方没跟上）在本轮已经被拦住两次。

## 仍未合并的部分

捕获流程的匹配 UI 与片单的 `workSearchOverlay` 仍是两套渲染代码（前者是流程内的
...
# R6 补丁 13：界面整理（命名 / 入口 / 溢出 / 片单卡片）

## 1. 侧边栏三项统一成四个字，图标各不相同
...
原来「作品书架」和「片单」共用 `shelf` 图标，扫一眼分不出哪个是哪个。
新增两个图标让形状本身承担区分：胶片格=已经收进来的作品，书签=还在排队等着看的。

FAB 与页面标题里的旧叫法一并跟上（返回观影轨迹 / 返回私人影库 / 返回候场片单）。

## 2. 新建片单从页内表单移进 FAB

那个表单常驻在片单列表下方，占掉一屏可观的空间，而「新建片单」是低频动作，
不该长期占版面。现在收进右下角 ＋ 的二级菜单，做成一个底部面板。
空状态文案也跟着改成「点右下角的 ＋ 新建一个」。

## 3. 「编辑这次观影」被撑出屏幕

**根因**：`.location-choice` / `.price-fields` / `.relation-toggle` 这些 grid / flex
容器的子项没有 `min-width: 0`。`datetime-local`、`number` 这类输入有较宽的**固有尺寸**，
在栅格里默认不肯收缩到它以下，于是整块把 sheet 撑宽；而 `.bottom-sheet` 是
`overflow: auto`，撑宽的结果不是裁切而是**横向滚动**，看起来就是"扩展到屏幕外"。

修法是统一按"允许收缩"处理（容器与子项都给 `min-width: 0`，输入统一
`box-sizing: border-box; width: 100%`），而不是给每个字段写死宽度——写死宽度
换个字段又会复发。另外给 `.bottom-sheet` 加 `overflow-x: hidden` 当最后一道保险。

## 4. 片单条目卡片重做

### 一处对用户方案的调整

用户提议「标题放海报下方」+「海报右边放理由」。这两条在同一个卡片里是冲突的：
前者是**网格**排版（豆瓣那个截图是网格视图，右边给的是剧照），后者是**列表**排版。
而 `reason` 是 R6 整个设计的核心，必须有横向空间容纳可长可短的文本——网格塞不下。
...
│  80px  │                              │
│  2:3   │ 重看《英雄归来》后觉得        │
│ ┌────┐ │ Michael Keaton 的秃鹫很好…   │
│ │看过│ │                              │
│ │8/09│ │                              │
└─┴────┴─┴──────────────────────────────┘
```

- 海报从 48px 放大到 80px，2:3 比例 + 圆角 + 投影，成为卡片的视觉锚点
- 「看过」做成压在海报右下角的**印章**（带最近观看日期），比灰色文字标签有存在感；
  **未看时什么都不显示**——片单里绝大多数条目本来就没看过，给它们都挂标签只是噪声
- 收藏时间（`entry.added_at`）常驻显示在标题下方，理由可以不填但收藏时间一定有
- 理由占据卡片主体，**点它直接进编辑**，不必绕菜单
- 编辑理由 / 上移 / 下移 / 移出四个操作收进右上角 `⋯` 的二级菜单——
  原来它们竖排在卡片最右侧，既占宽度又把卡片撑高，中间空出一大片
