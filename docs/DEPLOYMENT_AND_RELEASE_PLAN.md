...
## 结构调整阶段 R1–R5（2026-08-03 插入）

**方案文件：`docs/RESTRUCTURE_PLAN_R1-R5.md`（必读）**
**执行交接：`docs/handoff/R1_DATA_MODEL.md` ~ `R5_VERIFICATION.md`**

### 为什么插在 W6 之前

W6–W9 的情绪标签与记忆卡片都要挂在某个对象上。但当前 `work.id = \`work_${record.id}\``，Work 与 Record 一对一——同一部电影看两次会产生两个互不相识的作品条目。先做 W6–W9，46 个情绪标签和 44 种卡片类型会全部长在错误的对象关系上，之后返工成本更高。

### 窗口序列（严格串行，前一个未验收不得开下一个）

| 窗口 | 目标 | UI |
|---|---|---|
| R1 | Work 实体化、ViewingEvent 成为主干、Record 关系字段、存量自动迁移合并 | 无 |
| R2 | 「＋」→ 场景识别 → 确认卡 → 书写；剪贴板感知；跳过双分支 | 有 |
| R3 | 首页履历卡（海报＋场次＋制式勋章）；**移除轮换壁纸**；**记忆卡片改竖向连续流动** | 有 |
| R4 | 侧边栏、作品书架、作品页、观影履历、评价变迁、补充记录入口 | 有 |
| R5 | 端到端验证、迁移验证、安全回归、移动端截图、文档同步 | 验证 |

### 顺序变更说明

- **W4／W5（GitHub）推后到 R5 之后**：导出与推送的内容结构依赖新数据模型，先做会返工
- **W9 改范围**：原「初看／重看关系字段」已由 R1 吸收，W9 改为多场次编辑／删除／关联，以及补充记录的引用关系
- **W11／W12 新增**：年度报告（数据基础在 R1 已采好）、搜索与发布前打磨

### 红线变更（用户已决策 2026-08-03，共三项）

| 原红线 | 出处 | 变更后 |
|---|---|---|
| 票价默认脱敏移除 | HANDOFF_V2 §9 | **不再脱敏，正常记录并可导出**，存入 `ViewingEvent.ticket_price` |
| 记录卡保持不透明 | HANDOFF_V2 §7 | **整节废止**——轮换壁纸已移除，不透明约束失去前提 |
| 记忆卡片一次完整显示一张、不截断下一张 | HANDOFF_V2 §5／§3 | **改为竖向连续流动，下一张露出一角** |

票价维持强制移除的相邻项目：姓名、邮箱、手机号、订单号、取票码、二维码令牌、**支付方式与卡号**。票价是观影事实，支付信息是金融信息，两者区别对待。票价**不得**进入 AI 请求体。

「不使用紫色、紫色渐变、玻璃拟态、通用 AI SaaS 模板」**维持不变，仍是硬红线**——这也是半透明卡片方案被否掉的原因之一（半透明＋模糊即玻璃拟态）。

### 移除轮换壁纸（R3 执行）

用户实测：卡片铺满屏幕后壁纸基本看不到；日本主流影院 App 均未采用壁纸首页。R3 引入海报后视觉识别已由海报承担，R4 的作品书架提供另一条浏览路径，壁纸的两项价值都被取代。

**删除边界（务必看 `docs/handoff/R3_HISTORY_CARD.md` 第一部分）：** `apiBangumiImageUrl()`、Bangumi 图片代理、C2 的图片缓存策略**必须保留**（海报依赖）；`wallpaperSettingsOverlay()` **不能整个删掉**（它同时承载同步、AI、导出设置），只能改名并摘掉壁纸区块。

**替代方案：** 纪念日开屏庆祝（W14）。壁纸失败的原因不是视觉，而是它每天都在；纪念日一年只出现十几次，稀缺性使其成为事件而非噪音。数据（`Work.release_dates`、`ViewingEvent.viewed_on`）在 R1 一并采好。
